...
Este documento consolida o ambiente de desenvolvimento, os padrões de qualidade e as convenções de documentação de código do HECATE. Ele é a referência prática para desenvolvimento local, CI e revisão técnica.
...
O `composer.lock` é obrigatório. O bootstrap deve usar `composer install` e não deve executar `composer update`, `composer require` ou instalar pacotes do sistema de forma implícita.
...
Endpoints web são actions/handlers invocáveis em `src/Web`, com dependências fornecidas por DI. Não usar convenções Yii2 como `controllers/`, `models/`, `views/` e `widgets/` como referência arquitetural para código novo.
...
SonarQube é opcional e futuro; não é requisito para `make setup`, desenvolvimento local ou pipeline básico.
...
Evitar por padrão CSS e JavaScript específicos por página. Não usar `style=` ou `onclick=` como solução recorrente. Preferir assets compartilhados e `data-*` para parametrização de comportamento reutilizável.

## 8. PHPDoc, JSDoc e comentários

Tipos e nomes devem explicar o óbvio. PHPDoc e JSDoc devem acrescentar informação que a assinatura não expressa, como:

- array shapes e generics;
- efeitos colaterais;
- exceções relevantes;
- contratos de adapters e integrações;
- transações, concorrência e idempotência;
- formatos de dados externos.

Não adicionar PHPDoc redundante a toda classe ou método.

Comentários inline devem explicar o motivo de uma implementação, não apenas traduzir o código. Código comentado não deve permanecer no repositório.
...
composer qa
    ↓
testes funcionais/migrations quando aplicável
    ↓
commit / push
    ↓
CI
